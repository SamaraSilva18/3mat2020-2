...
module.exports = class BinarySearchTree {
    ...
    insert(value) {
        ...
        // Percorre a árvore para enconrar o local correto de inserção
    ...
    insertNode(node, root) {
        // O valor a ser inserido é menor que o valor que está na raiz
        if(node.data < root.data) {
            // Possibilidade 1: Não há nada à esquerda do nó da raiz
            // Encontramos o ponto de inserção
            if(root.left === null) root.left = node
            // Possibilidade 2: o nodo à escerda da raiz já está ocupado
            // Chamamos então, recursivamente, a própria função, ALTERANDO A RAIZ
            // DE REFERÊNCIA, para continuar a encontrar o local de encaixe
    ...
    preOrderTravessal(fnCallback, root = this.root) {
        if(root !== null) {
            fnCallback(root) // Visita a raiz
            this.preOrderTravessal(fnCallback, root.left) // Visita recursivamente o lado esquerdo
            this.preOrderTravessal(fnCallback, root.right) // Visita recursivamente o lado direito
        }
    }

    // Percurso pós-ordem (post-order)
    postOrderTravessal(fnCallback, root = this.root) {
        if(root !== null) {
            this.postOrderTravessal(fnCallback, root.left) // Visita recursivamente o lado esquerdo
            this.postOrderTravessal(fnCallback, root.right) // Visita recursivamente o lado direito
            fnCallback(root) // Visita a raiz
        }
    }
    ...
    // true se existe ou falze se não existe o nodo
    searchNode(root, key) {
        // Caso 1: a árvore está vazia
        if(root === null) return false

        // Caso 2: o key é menor que o data no node -> parte para a esquerda
    ...
    removeNode(root, key) {
        ...
        // Caso 4: o key é igual ao data da raiz -> ENCONTROU o nodo para remoção
            ...
            // Caso 4d: excsão de nodo de grau 2
            ...
            root.right = this.removeNode(node.right, minRight.key)
            return root
        }
    }
}